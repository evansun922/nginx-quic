...
QuicNgxBackend::QuicNgxBackend() : cache_initialized_(false) {}

QuicNgxBackend::~QuicNgxBackend() {}

bool QuicNgxBackend::InitializeBackend(const std::string& cache_directory) {
  cache_initialized_ = true;
  return true;
}

bool QuicNgxBackend::IsBackendInitialized() const {
  return cache_initialized_;
}

void QuicNgxBackend::FetchResponseFromBackend(
...
void QuicNgxBackend::CloseBackendResponseStream(
    QuicSimpleServerBackend::RequestHandler* quic_stream) {
  QuicNgxStream* ngx_stream = reinterpret_cast<QuicNgxStream*>(
                              static_cast<QuicSimpleServerStream*>(quic_stream));
